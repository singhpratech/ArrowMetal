...
- Polars engine version checks: `TESTED_POLARS` is a tuple, `("1.44.1", "1.44.2")`; the full engine
  ...
  engine, stays with Polars whole with the report line `The plan holds an unknown node <kind> in
  polars <version>, so the whole plan stays with Polars.` instead of an error.
...
- CPU/GPU router: `sum`, `min`, `max`, `compare`, `add`/`subtract`/`multiply`, `filter` (by mask and
  ...
  is generated by `Benchmarks/router_table.py --from-check` from a `Benchmarks/router_check.py` run of the
  shipped loops (`Benchmarks/results/router_check_2026-09-24.csv`), `multiply` with a row of its own; a
