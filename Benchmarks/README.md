...
| `PYTHONPATH=python python Benchmarks/polars_engine_bench.py [--sizes 2000000,50000000] [--iters 5]` | `lf.collect(engine=am.MetalEngine())` against Polars' in-memory and streaming engines on the eight `engine_bench.py` shapes and 37 more group-by, aggregate, join, sort, `unique` and String shapes as LazyFrames, import included, results checked against Polars (see `../docs/POLARS.md`). Results: `results/polars_engine_bench_2026-09-26-quiet3.csv` (earlier: `results/polars_engine_bench_2026-09-26-quiet.csv`, `results/polars_engine_bench_2026-09-26.csv`, `results/polars_engine_bench_2026-09-24.csv`, `results/polars_engine_bench_2026-09-23_provisional.csv`). |
| `PYTHONPATH=python python Benchmarks/polars_engine_bench.py --crossover --sizes ... --scan --scan-rows ...`, then `python Benchmarks/polars_engine_crossover.py CSV` | The crossover sweep and its fit: every case at several sizes through Polars' two engines and `MetalEngine(shapes="all")`, fitted per shape class into `python/arrowmetal/_engine_crossovers.py`, the table `MetalEngine()`'s default reads (`--check` fails when the two disagree). Results: `results/polars_engine_crossover_2026-09-26-quiet.csv` (earlier: `results/polars_engine_crossover_2026-09-26.csv`). |
