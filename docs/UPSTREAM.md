...
| Apache Arrow Go v18.7.0 | no allocator promises an alignment: `NewGoAllocator` buffers inherit Go's 8 KiB spans, so page-aligned (16 KiB) import is 13/20 at 1M and 5/20 at 10M int64; the allocator the cdata docs recommend when C retains pointers is behind the `ccalloc` build tag and a C++ Arrow install | `TestAllocatorAlignment`; `arrowmetal.PageAlignedAllocator` is the binding's own answer | github.com/apache/arrow-go, not yet filed; draft ready | open, candidate |
| Apache Arrow Go v18.7.0 | `compute.NewDatumWithoutOwning` returns the same `Datum` type as an owning datum, so an over-release compiles and frees buffers under a live array with no error; its doc comment is truncated (`datum.go:277`, "convenience function.+-") | reproduced while writing the Go timing harness; [GO.md](GO.md) | github.com/apache/arrow-go, not yet filed; draft ready | open, candidate |
