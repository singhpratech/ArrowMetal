...
| Apache Arrow Go v18.7.0 | no allocator promises an alignment: `NewGoAllocator` buffers inherit Go's 8 KiB spans, so a buffer starts at offset 0 or 8192 within a 16 KiB page and page-aligned import is a coin flip whose odds depend on heap fragmentation (measured between 5/20 and 15/20 across runs); the allocator the cdata docs recommend when C retains pointers is behind the `ccalloc` build tag and a C++ Arrow install. Related and already closed: [apache/arrow-go#70](https://github.com/apache/arrow-go/issues/70) (the C Data Interface storing unpinned Go pointers, which `runtime.Pinner` addresses) | `TestAllocatorAlignment`; `arrowmetal.PageAlignedAllocator` is the binding's own answer to alignment | filed 2026-09-08: [apache/arrow-go#1297](https://github.com/apache/arrow-go/issues/1297), scoped to alignment so it does not duplicate #70 | filed, open |
