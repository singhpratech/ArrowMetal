...
| `test_json.py` | 34 (437 collected) | `pyarrow.json.read_json` on every probe, explicit-schema case, generated, truncated and mutated file (values, nulls, types, or the error text), plus one test per documented difference ([JSON.md](JSON.md)) |
