...
| `test_polars_engine.py` | 58 | `MetalEngine` (tier 4 of [POLARS.md](POLARS.md)) against Polars' own collect of the same LazyFrame, over `test_differential.py`'s generators at five sizes, three null ratios, sliced, special-value, two-chunk and `DataFrame.slice` frames; each fallback with its reason; the Polars surfaces and IR version it relies on; four strict xfails pinning engine behaviours it works around; one case reruns `test_polars.py` and `test_lazy.py` with every collect also run through the engine (`metal_engine_everywhere.py`) |
