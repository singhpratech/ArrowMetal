...
| `test_lakehouse.py` | 19 | `deltalake`'s `to_pyarrow_table` and pyiceberg's `scan().to_arrow()`: the recorded reads in `expected.json`, the committed tables live (types included), and tables generated at test time read with time travel and with filters of every comparison over every column type; the differences from the reference readers that [LAKEHOUSE.md](LAKEHOUSE.md) lists. The live comparisons skip when `deltalake` or `pyiceberg` is not installed |
