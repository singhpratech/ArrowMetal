...
| `test_parquet_nested.py` | 30 | `pyarrow.parquet.read_table` on nested fixtures from pyarrow, DuckDB and Polars, value for value and type for type; the stored Arrow schema's types, field metadata and schema metadata; page skipping and bloom filters against pyarrow's exact matches with the index and filters on and off; damaged nested files and damaged page indexes |
