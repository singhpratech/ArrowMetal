...
| `test_csv.py` | 24 | `pyarrow.csv.read_csv` on generated files: identical tables (floats bit for bit) or identical error messages, one case per inference and parsing rule, every option, 60 seeded random files, block-boundary straddles, the type decided past the inference sample, files written by Polars and DuckDB ([CSV.md](CSV.md)) |
