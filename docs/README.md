...
| [CROSSOVER.md](CROSSOVER.md) | The crossover: from what row count the GPU path is ahead of the fastest CPU idiom, per operation and per family, and ahead of the CPU loop the engine's router would run instead; generated by `Benchmarks/crossover.py` from a size sweep of the matrix and `arrowmetal-bench crossover` |
