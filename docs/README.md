...
| [CROSSOVER.md](CROSSOVER.md) | The crossover: from what row count the GPU path is ahead of the fastest CPU idiom, per operation and per family, and ahead of a single-core CPU loop (the bench's, from which the router's table was fitted); generated by `Benchmarks/crossover.py` from a size sweep of the matrix and `arrowmetal-bench crossover` |
| [APPLE_REPORTS.md](APPLE_REPORTS.md) | The three Metal findings reported to Apple through Feedback Assistant on 2026-09-20 (64-bit atomics against the Feature Set Tables, pipeline creation on GitHub's virtual GPU, CPU-to-GPU visibility for a persistent kernel): what each says, what was measured, what was asked, and the FB numbers; no attachments, no personal data |
