...
`cargo test --release` runs 43 tests and 3 doc-tests. Release matters: this project has hit one
release-only miscompile on the Swift side, and the sweeps run at 1,000,001 elements, which is slow
to build and run unoptimised.

| File | Tests | Oracle |
|---|---|---|
| `arrowmetal/tests/compute.rs` | 28 | `arrow::compute` on the same array — `sum`/`min`/`max`, the six comparisons, `filter`, `sort`, `take`, `slice`, `cast`. `group_by` has no counterpart in the `arrow` crate (hash aggregation lives in DataFusion), so its oracle is a plain `HashMap` fold. |
| `arrowmetal/tests/plan.rs` | 7 | the same plans assembled by hand from arrow-rs kernels |
| `arrowmetal/tests/copy_rule.rs` | 4 | measured pointer alignments, not assumptions |
...
* **end to end** — import from arrow-rs, run, export back. This is what *one* operation on an
  arrow-rs array costs.
