...
writer emits uncompressed, little-endian bodies of classic types only. The reader also takes big-endian sources, the view types (materialised to the classic layouts) and the `arrow.fixed_shape_tensor` extension type; IPC tensor messages are refused;
