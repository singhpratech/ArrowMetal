...
extension MetalStringArray {
    ...
    /// Ops understood by `StringExtraSource`. The numbering is local to that file.
    enum ExtraTransform: Int {
        case utf8Swapcase = 0
        case utf8ZeroFill = 1
    }

    /// The two-pass driver for the transforms in `StringExtraSource`.
    func extraTransform(_ op: ExtraTransform, arg1: [UInt8] = [], p1: Int = 0) throws -> MetalStringArray {
        ...
        func pso(_ f: String) throws -> MTLComputePipelineState {
            try ctx.pipeline(source: StringExtraSource.source, function: f, cacheKey: "strx/\(f)")
        }
        if n > 0 {
